...
for t in range(0,gf.TILES):
		for s in range(0,gf.STRIPS):
				...
				print "../outputs/instructions/If_%d_%d" %(t,s),"r"
				...
				for j in range(0,len(l)):
						...
						temp = l[j].split(":")[1]	#Extract the roach formatted data only
						l[j]=temp
				...
				i.close()
print "TOTAL instruction Count: ",count
...
if count > len_ichunk:
		...
		commandH = gf.bin_to_hex(command) 
		print"command2: ", commandH
		commandH = gf.formatH_for_roach(commandH)
		print"command3: ", commandH
		LIprogram=commandH+string+ZERO_WORD_RH+ZERO_WORD_RH
		print "LIprogram: ",LIprogram[0:100]
		Nosys_commands = 1
