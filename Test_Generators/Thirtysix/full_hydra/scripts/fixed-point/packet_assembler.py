...
dataF.close()
os.system(r"sed -i 's/\\x//g' ../outputs/data.txt")	#Remove '\x' tags in file, in there cause it's more trouble than it's worth to remove them on generation
print("Packed WHOLE program which fits into %d packets" %(pcounter2))

#Write simulation inputs to file:
simF = open("../outputs/sim.txt","w")
for i in range(0,len(Program),32):	#8=length of a 64bit roach word
		line1="sim_rx_data <= X\""+Program[i:i+32]+"\";"
		line2="wait until sim_rx_ack = '1' and rising_edge(clk);"
		simF.write(line1+"\n")
		simF.write(line2+"\n")
simF.close()
os.system(r"sed -i 's/\\x//g' ../outputs/sim.txt")	#Remove '\x' tags in file, in there cause it's more trouble than it's worth to remove them on generation
