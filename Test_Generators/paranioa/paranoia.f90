program paranoia_f90_s
!
!******************************************************************************
!
!! PARANOIA_F90_S investigates floating point arithmetic under FORTRAN 90.
!
!
!  Local Parameters:
!
...
10001   format('  Restarting from milestone ',i5,'.')
        ...
        if (from >= 105 .and. from <= 109) go to 10100
        if (from == 115) go to 10100
        if (from >= 120 .and. from <= 125) go to 10100
        if (from == 131) go to 10100
        if (from == 161) go to 10200
        if (from >= 201 .and. from <= 205) go to 10200
        if (from == 211) go to 10300
        if (from == 212) go to 10300
  write ( *, * ) ' '
  write ( *, * ) 'PARANOIA_F90_S - Error!'
  write ( *, * ) '  Unrecognized restart milestone.'
  stop

10000  continue

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'PARANOIA_F90_S'
  write ( *, '(a)' ) '  A "paranoid" program to diagnose floating arithmetic.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  This version written in Fortran 90,'
...
881 continue
  ...
  call small_int ( defect, fails, fp0, fp1, fp2, from, ieee, miles, &
    radix, sdefct, ulppls )
...
!
  call radx ( defect, fails, flaws, fp0, fp1, fp2, fp4, fp9, half, &
...
1680 continue
  ...
  call extra ( fails, fp0, fp1, fp2, fp3, fp32, fp4, fp8, half, &
...
!
  ...
  call guard ( defect, divgrd, fails, fp0, fp1, fp2, fp27, fp3, fp9, &
...
!
  call round ( a1, divgrd, fails, flaws, fp0, fp1, fp2, fp3, fp9, half, &
...
!
  call commute ( defect, fp0, fp3, half, ulpmin, ulppls )
  miles = 70
!
!  Test square root.
!
3959    continue
  ...
  call square ( a1, defect, fails, fp0, fp1, fp2, fp4, fp8, fp9, from, &
...
10100   continue
  ...
  call underflow ( c1, fp0, fp1, fp2, fp3, fp32, fp8, from, h1, half, ieee, &
...
10200   continue
  ...
  call overflow ( c1, defect, fails, flaws, fp0, fp1, fp2, from, h1, &
...
10300   continue
  ...
  call zeros ( from, fp0, fp1, ieee, miles )
  from = 0
  call check_write ( ieee, miles )
  miles = miles + 1
  ...
  if (fails+sdefct+defect+flaws > 0) go to 6270

  if (r1+r2+r3+r4 < fp4) go to 6260

  if (sticky < fp1 .or. (radix-fp2)*(radix-fp9-fp1) /= fp0) &
           go to 6250
...
6270    continue
  ...
  if (fails+sdefct+defect == 0 .and. flaws > 0) then
    write ( *, '(a)' ) ' '
...
6280    continue
  ...
  if (fails+sdefct == 0 .and. defect > 0) then
    write ( *, '(a)' ) ' '
...
subroutine commute ( defect, fp0, fp3, half, ulpmin, ulppls )
!
!******************************************************************************
!
...
subroutine extra ( fails, fp0, fp1, fp2, fp3, fp32, fp4, fp8, half, &
  onemin, radix, sdefct, ulpmin, ulppls )
!
!******************************************************************************
!
...
!
  ...
  if ( z1 >= ulpmin .or. z2 >= ulppls ) then
    fails = fails + 1
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'Failure:'
...
subroutine guard ( defect, divgrd, fails, fp0, fp1, fp2, fp27, fp3, fp9, &
  half, mulgrd, onemin, radix, sdefct, subgrd, ulpmin, ulppls, w )
!
!******************************************************************************
!
...
!
  ...
  if ( onemin /= fp1 .and. onemin - fp1 >= fp0 ) then
    sdefct = sdefct + 1
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'Very serious defect:'
...
subroutine newd ( x, z1, q, z, d, fp0, fp1, fp2, half, radix )
!
!******************************************************************************
!
!! NEWD ???
...
subroutine overflow ( c1, defect, fails, flaws, fp0, fp1, fp2, from, h1, &
...
!  Parameters:
!
!    FROM, counts the milestones previously reached.
!
!    MILES, the number of milestones passed.
!
  implicit none
!
  real c1
...
!
  if ( from /= 0 )then
    ...
    if (from >= 201 .and. from <= 205) go to 5999
    write ( *, * ) ' '
...
5660    continue
  ...
  if ( -v >= v .or. -sat >= sat .or. -uflthr >= v .or. uflthr >= v ) then
    fails = fails + 1
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'Failure:'
...
5700    continue
  ...
  if ( z /= fp0 ) then
    ...
    if ( y/(fp1-radix * nulps) < z .or. y > (fp1+radix*nulps)*z ) then
...
!
  if ( radix == 2. .and. precis == 56.0 .and. &
      phony0 /= 0.0 .and. -fp0 /= fp0 ) then
    ...
    do
      ...
      if ( v9 < (fp1-fp2*radix*nulps)*z .or. v9 > z ) then
  ...
  if (x * y < fp1 .or. x > y ) then
    if ( x * y < ulpmin .or. x > y / ulpmin ) then
      defect = defect + 1
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'Defect:'
...
subroutine partuf ( z, zname, defect, fp0, fp1, fp2, ieee, miles, partu, &
...
!
  ...
  if ( .not. restrt ) then
    ...
    else if ( dummy < fp1 .or. dummy > fp2 ) then
      partu = 1
      sdefct = sdefct + 1
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) '  Serious defect!'
...
!  
  ...
  if ( n > 0 ) then
    write ( *, * ) ' '
    write ( *, * ) '  An error like this may invalidate financial'
    write ( *, * ) '  calculations involving interest rates.'
  end if
...
subroutine radx ( defect, fails, flaws, fp0, fp1, fp2, fp4, fp9, half, &
  miles, onemin, precis, radix, sdefct, ulpmin, ulppls, w )
!
!******************************************************************************
!
...
!  If radix >= 2, try to find the precision (# significant digits)
!
  else
...
subroutine round ( a1, divgrd, fails, flaws, fp0, fp1, fp2, fp3, fp9, half, &
  miles, minus1, mulgrd, onemin, r1, r2, r3, radix, sticky, subgrd, &
  ulpmin, ulppls )
!
!******************************************************************************
...
2760    format('  when x = ',e16.8,'  and  y = ',e16.8)
  ...
  x = (half+ulpmin)*ulppls
        ...
        z = fp1+y
        t = fp1+x
        if (z-fp1 > fp0 .or. t-fp1 < ulppls) go to 2890
    z = t+y
        y = z-x
...
subroutine small_int ( defect, fails, fp0, fp1, fp2, from, ieee, miles, &
  radix, sdefct, ulppls )
!
!******************************************************************************
!
...
951     continue
  ...
  call partuf ( tempz, charz, defect, fp0, fp1, fp2, ieee, miles, partu, &
    radix, restrt, sdefct, ulppls )

960     continue

  if ( 4.0e0 + 2.0e0 * (-2.0e0) /= 0.0e0 ) then
    fails = fails + 1
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'Failure:'
  ...
  if ( (4.0e0-3.0e0)-1.0e0 /= 0.0e0 ) then
    fails = fails + 1
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'Failure:'
  ...
  if ( minone+1.0e0 /= 0.0e0 .or. &
    1.0e0+minone /= 0.0e0 .or. &
    minone + abs ( minone ) /= 0.0e0 .or. &
    minone + minone * minone /= 0.0e0 ) then
    fails = fails + 1
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'Failure:'
    write ( *, '(a)' ) '  Violation of -1 + 1 = 0.'
  ...
  if ( half+minone+half /= 0.0e0 ) then
    fails = fails + 1
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'Failure:'
  ...
  if ( temp32-temp27-4.0e0-1.0e0 /= 0.0e0 ) then
    fails = fails + 1
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'Failure:'
  ...
  temp80 = temp240 / 3.0e0
  temp = temp80 - 4.0e0 * temp20
...
subroutine square ( a1, defect, fails, fp0, fp1, fp2, fp4, fp8, fp9, from, &
  half, ieee, miles, minus1, onemin, precis, r4, radix, sdefct, &
  ulpmin, ulppls, w )
!
!******************************************************************************
...
!
!  If gcd(y,b) > 1 then skip over y;  else
!
      if ( q <= fp1 ) then
        z2 = y2
        z = y
...
!
  do
    ...
    call newd (x, z1, q, z, d, fp0, fp1, fp2, half, radix )

    if ( ulppls * d >= onemin ) then
      exit
    end if
...
subroutine underflow ( c1, fp0, fp1, fp2, fp3, fp32, fp8, from, h1, half, &
...
4631    continue
  ...
  call partuf ( z, charz0, defect, fp0, fp1, fp2, ieee, miles, partu, &
    radix, restrt, sdefct, ulppls )
!
...
4941    continue
  ...
  call partuf ( z, chare0, defect, fp0, fp1, fp2, ieee, miles, partu, &
    radix, restrt, sdefct, ulppls )
...
5220    continue
  ...
  y = -aint(half - 240.0 * log(uflthr) / log(h1)) / 240
  y2 = y + y
  write ( *,5240)h1,y
...
5245    format(' only underflow should afflict the expression'/10x,'(', &
  ...
  if ( v9 < fp0 .or. v9 > (radix+radix*nulps)*uflthr ) then
    sdefct = sdefct + 1
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'Serious defect:'
    write ( *, '(a,e16.8)' ) &
...
5360    format(' testing  x^((x+1)/(x-1)) vs. exp(2) = ',e16.8,'  as  x -> 1.')
  ...
  do
    ...
    do i = 1, numtry
      ...
      z = x - ( 1.0 / radix )
      z = (x+fp1)/(z-(fp1-(1.0/radix)))
      q = x**z-exp2

      if ( abs ( q ) > 240.0 * ulppls ) then
        accur = 1
        defect = defect + 1
        temp = +(x-(1.0/radix))-(fp1-(1/radix))
        write ( *, '(a)' ) ' '
        write ( *, '(a)' ) 'Defect:'
        write ( *,5425)temp,z
...
subroutine zeros ( from, fp0, fp1, ieee, miles )
!
!******************************************************************************
!
