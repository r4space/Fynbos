...
#How the packets are structured is a further test,used to verify Hydra's communication system.
...
LI_commandH = commandH
print "LI command1: ", commandH
...
###################################################################################
ProgramData = LDprogram+LIprogram
ProgramCmds = run_commandH+ZERO_WORD_RH+ofld_commandH+ZERO_WORD_RH
...
pcounter2=0
packet=''
#Write data and instructions
for i in range(0,len(ProgramData),17920):	#17920=16*1120, 16=length in the program string of a 64bit roach word, 1120=packet size of 1121 64bit words-1 to insert a starting zero word
		packet=packet+ProgramData[i:i+17920]
...
#Write run/ofld/etc commands
if len(ProgramCmds)>=17920:
		print "Error, ProgramCmds too big to fit in one packet, new approach needed"
		sys.exit();

packet=ProgramCmds
dataF.write(packet+"\n")
pcounter=pcounter2+1
...
for i in range(0,len(Program),16):	#16=length of a 64bit roach word
		...
		simF.write(line2+"\n")
simF.close()
print("Packed WHOLE program into a simulation inputs file")
