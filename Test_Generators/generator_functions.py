...
DROWS =1024#Number of data registers per strip
IROWS =1024	#Number of execution rows and also the number of instruction registers per strip, must allways be <DROWS
STRIPS = 6	#Number of strips per tile
TILES = 5	#Number of tiles
div_strips =27	#Number of strips with division capabilities
D_len = 32		#Number of bits in a data word
D_we = 	8		#Number of bits in the exponent 
D_wf = 	23		#Number of bits in the fraction s.t D_we+D_wf=D_len-1(sign bit)

STRIP_Ad_bits=int(m.ceil(m.log(STRIPS,2)))
TILES_Ad_bits=int(m.ceil(m.log(TILES,2)))
DADDR_BITS = int(m.ceil(m.log(DROWS,2)))	#Number of bits used to address a register
IADDR_BITS = int(m.ceil(m.log(IROWS,2)))	#Number of bits used to address a register
INS_ADDR_CD=DADDR_BITS+STRIP_Ad_bits+TILES_Ad_bits	#DREGS_Ad_bits+strip_Ad_bits+tile_ad_bits
INS_ADDR_CI=IADDR_BITS+STRIP_Ad_bits+TILES_Ad_bits	#IREGS_Ad_bits+strip_Ad_bits+tile_ad_bits
...
#LI and LD are the largest possible command instructions determined by the number of registers in either the data or instruction memory
#The maximum command length is therefore controled by the number of bits needed to control which ever or these is lngest
#This depends on memory depths and number of strips
max_load_count_length = int_to_bin_length(TILES*STRIPS*max(DROWS,IROWS))
SC_len=INS_ADDR_C+3+max_load_count_length  #Number of bits in a system command
if SC_len > 64:
		print '\033[1;41mThis is a problem, a command instruction cannot be bigger than 64 bits\033[1;m'
...
####Convert a binary string of anylength into a Hexidecimal string of a given length(which is divisable by 4)##########
...
#returns an array [row, strip, tile] address values in integers
...
#Different ones are needed for strip and tile selector muxs, sel0a and sel1 are strips, selq is a interTile
def selS_string (sel, default):
		if sel == -1:
				sel=default
		return int_to_bin(sel,STRIP_Ad_bits)
def selT_string (sel, default):
		if sel == -1:
				sel=default
		return int_to_bin(sel,TILES_Ad_bits)
