...
####NOTES:
#System must contain at least the minimum of strips and tiles (3,2) and 16 data rows and 8 instruction rows)
#Values higher than such will contain an operation for the number of IROWS there are where the same address is used for both operands and stored in the same (implied) location.
#For data rows higher than the last IROW, a value is loaded and offloaded, no operation is done to it

if gf.DROWS != 2*gf.IROWS:
		print "ERROR: the number of data rows must be dounle the number of instruction rows for this script to be run"
		sys.exit()
...
for s in range(0,3):
		...
		#Fill the remaining memory with knowns which won't be used				
		for r in range(gf.IROWS,gf.DROWS):
				varI=random.randrange(0,100)
				varFP=FP.custom_floating_point_creator (varI,gf.D_wf,gf.D_we)
				d_wordH = gf.bin_to_hex_l(varFP,64)
