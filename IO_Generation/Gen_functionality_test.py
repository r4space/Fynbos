...
addr2	= gf.int_to_bin(r,gf.DADDR_BITS)
sel0	= gf.int_to_bin(0,gf.STRIP_Ad_bits)
sel1	= gf.int_to_bin(s,gf.STRIP_Ad_bits)
selq	= gf.int_to_bin(t,gf.TILES_Ad_bits)
...
addr1	= gf.int_to_bin(r,gf.DADDR_BITS)
addr2	= gf.int_to_bin(r,gf.DADDR_BITS)
sel0	= gf.int_to_bin(1,gf.STRIP_Ad_bits)
sel1	= gf.int_to_bin(s,gf.STRIP_Ad_bits)
selq	= gf.int_to_bin(t,gf.TILES_Ad_bits)
...
for t in range(0,gf.TILES):
		for r in range(cr,cr+NO_inter_strip):	
				...
				for s in range(0,gf.STRIPS):
						...
						sel0	= gf.int_to_bin(sel0_range[s],gf.STRIP_Ad_bits)	#Set to prearranged other strip
						sel1	= gf.int_to_bin(s,gf.STRIP_Ad_bits)	#Operand1 set to local (it's meaningless)
...
for t in range(0,gf.TILES):
		for s in range(0,gf.STRIPS):
				...
				sel0	= gf.int_to_bin(s,gf.STRIP_Ad_bits)	#Set to local strip
				sel1	= gf.int_to_bin(s,gf.STRIP_Ad_bits)	#Operand1 set to local (it's meaningless)
				selq	= gf.int_to_bin(t,gf.TILES_Ad_bits)	#Set to local tile    
...
for s in range(0,gf.STRIPS):
		...
		for r in range(cr,cr+NO_inter_tile):
				...
				for t in range(0,gf.TILES):
						...
						sel0	= gf.int_to_bin(s,gf.STRIP_Ad_bits)	#Set to local strip
						sel1	= gf.int_to_bin(s,gf.STRIP_Ad_bits)	#Operand1 set to local (it's meaningless)
						selq	= gf.int_to_bin(selq_range[t],gf.TILES_Ad_bits)	#Set the selq to pre shifted     
...
for t in range(0, gf.TILES):
		for s in range(0, gf.STRIPS):
				...
				for r in range(cr,cr+11):
						...
						sel0	= gf.int_to_bin(s,gf.STRIP_Ad_bits)	#set sel's to the current strip
						sel1	= gf.int_to_bin(s,gf.STRIP_Ad_bits)
						selq	= gf.int_to_bin(t,gf.TILES_Ad_bits)	#Set the selq to the current tile    
