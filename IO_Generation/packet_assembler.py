...
#	at the correct locations or skips to just adding 1 load instructions command at the front of the string if it does fit.

len_dchunk = pow(2,gf.SC_len-gf.INS_ADDR_CD-3)-1	#Eg: 65536 = 2^16, 16 counter bits available in the LOAD_DATA command
print "len_dchunk: ", len_dchunk
print "count: ", count
...
##NOT SURE THIS STILL WORKS, haven't tested it POST RE-PARAMETERISING, usually just use the ELSE statement
len_ichunk = pow(2,gf.SC_len-gf.INS_ADDR_CI-3)-1	#Eg: 65536 = 2^16, 16 counter bits available in the LOAD_INSTR command
if count > len_ichunk:
		...
		for i in range (0,Nosys_commands):
				...
				new_count= gf.int_to_bin(new_count,gf.SC_len-gf.INS_ADDR_CI-3)#How many words to load
		...
		start= gf.int_to_bin(0,gf.INS_ADDR_CI) #address = IADDR_BITS+3bits for strip+3bits for tile
		count= gf.int_to_bin(count,gf.SC_len-gf.INS_ADDR_CI-3)	#How many words to load: goes into as many bits are left after the start address and the 3bits for setting the commadn type(LI in this case) hence 9
...
end = gf.int_to_bin(endrow,gf.IADDR_BITS)	#End at the last row by default 
print "end: ", end
print "start: ", start
...
cmd="100"
tile = gf.int_to_bin((gf.TILES-1),gf.TILES_Ad_bits)
strip = gf.int_to_bin((gf.STRIPS-1),gf.STRIP_Ad_bits)
row = gf.int_to_bin(endrow,gf.DADDR_BITS)
start = row+strip+tile
numregs = gf.int_to_bin(endrow+1,gf.SC_len-gf.INS_ADDR_CD-3)	#New - now used to set DREGs for the offload command
