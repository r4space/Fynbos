...
#						print "			Strip: ",s, "s_index: ",s_index
						...
						while s_index < s:
								...
								opcode=opcode_dict["NOP_I4"]
								#Set sel's to it's local strip/tile value as a matter of default which is the purpose of the index parameters
								sel0=gf.selS_string(-1, s_index) 
								sel1=gf.selS_string(-1, s_index)
...
#						print "t%ds%dr%d"%(t,s,r), ": ",opcodeT,":",opcode
						...
						sel0=gf.selS_string(int(operator.getAttribute("input_strip_0")), s_index) 
						sel1=gf.selS_string(int(operator.getAttribute("input_strip_1")), s_index)
						...
						selq=gf.selT_string(int(importer.getAttribute("source_tile")),t_index)
...
#Fill all remaining registers with zeros and nops
print "******************************************"
for t_index in range (0,TILES):
		for s_index in range (0,STRIPS):
				#Put Zeros in unused data registers
				for DataRow in range(endrow+1, gf.DROWS):
						data=FP.custom_floating_point_creator(0.0,gf.D_wf,gf.D_we)
						H_data=gf.bin_to_hex(data)
						H_data=gf.formatH_for_roach(H_data)
						Dword="Address %d:"%(DataRow) + H_data
						zD[t_index][s_index].write(Dword+"\n")	#Write the word to file
						count = count+1
#						print "NOP Dwards: ", H_data

				#Put NOPs in unused instruction registers	//This is only necessary because the loader needs something to load
				for InstrRow in range(endrow+1, gf.IROWS):
						opcode=opcode_dict["NOP_I4"]
						#Set sel's to it's local strip/tile value as a matter of default which is the purpose of the index parameters
						sel0=gf.selS_string(-1, s_index)
						sel1=gf.selS_string(-1, s_index)
